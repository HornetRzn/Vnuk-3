...
const settings = {
  ...
  keywords: {
    'в рот': {
      texts: [
        ...
        'Я на пл. Победы (ближе к клубу «Deep») вчера ночью оформился с рандомным незнакомцем 👀 бывает же такое…',
      ...
      stickers: [
        'CAACAgIAAxkBAAEBIilnybXMx5P1glipjfoF54XEk6ObAgACPgQAAtoqiEW0evyXoXMyTjYE',
        'CAACAgEAAxkBAAEBIj9nydntX-FbHGuMsXQYC3wOBqS1pgACywQAAucFiUUDrQ6MGpGJ-DYE'
      ]
    },
    'сообщали': {
      texts: [
        ...
        'Читаю эти новости и понимаю, как хочется близости с тем, кому я уже и не нужен, по большому счёту 🤧',
      ...
      stickers: [
        'CAACAgEAAxkBAAEBIkNnydp0WA9y6WvAJ4jYyazfkh9SkgACQgcAAtzFiUUGHxbLwu8XpzYE',
        'CAACAgEAAxkBAAEBIkVnydrZrIHYiixFyhvpxuIwqsMudQACdAUAApwGiEVsVYAE9s962jYE'
      ]
    },
    'пасс': {
      texts: [
      ...
      stickers: [
        'CAACAgEAAxkBAAEBIkFnydof_YNGvOCCLr6ALGrPgp_2BQACEQUAAjukiUXFFXd_IG6ZGzYE'
      ]
    },
    'авто': {
      texts: [
        ...
        'В тачке максимум отсос можно оформить. Анально в этих апартаментах не развернуться 😵‍💫',
        ...
        'У тебя и было-то всего двое парней за всю жизнь. И те с рязанских окраин, обиженных всеми гей-богами Вселенной 😏',
      ...
      stickers: [
        'CAACAgEAAxkBAAEBIk1nydttIprLic-RnT0kM0lZci0GBAACfwgAAvZhiEULB5CW30ssjzYE'
      ]
    },
  ...
  farewellMessages: [
    ...
    'Ты пока из Рзн ведь никуда не собираешься? Я через пару дней напишу, можт. Поебёмся 😈',
    ...
    'Что-то ты мне подозрительным кажешься. Долгое пиздабольство напрягает прям 👀 нужно быть осторожнее. Вот тут наши парни писали уже 👉🏻 https://t.me/hornetrzn/654 👈🏻 пойду поосторожничаю малость',
  ...
  dialogResponses: {
    ...
    step2: [
      ...
      'Вот так всегда. С кем бы я ни говорил…',
...
// Проверка реплая на стикер бота
const isReplyToBotSticker = (ctx) => {
  const reply = ctx.message?.reply_to_message;
  return reply?.from?.id === ctx.botInfo.id && reply.sticker;
}
...
bot.on('message', async (ctx) => {
  try {
    ...
    // Обработка ответов на стикеры
    if (isReplyToBotSticker(ctx)) {
      const reply = getRandomResponse(settings.stickerReplies);
    ...
    if (!session.inAIMode && !isReplyToBot(ctx)) {
      ...
      if (keyword) {
        const responses = settings.keywords[keyword];
        const sticker = Array.isArray(responses) 
          ? getRandomResponse(responses)
          : getRandomResponse(responses.stickers || []);
        
        if (sticker) {
          await ctx.replyWithSticker(sticker, replyOpt);
        } else if (responses.texts) {
          await ctx.reply(getRandomResponse(responses.texts), replyOpt);
        }
      ...
      switch(session.step) {
        ...
        default:
          await ctx.reply(getRandomResponse([
            "Вот извини, малыш 😔 немного не до тебя сейчас. Позже договорим, если не забудем. Чмокаю тебя в попку 💋",
