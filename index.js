...
const groupActivations = new Map(); // Для хранения активаций
...
const settings = {
  ...
  secretCommand: {
    command: '/secret_mode', 
    allowedUsers: [6141712830], // Замените на ваш цифровой ID
    cooldown: 60 * 60 * 1000, // 1 час между активациями
    errorMessage: '❌ Формат: /secret_mode Ваш текст'
  },
  ...
  ai: {
    model: "llama3-70b-8192",
     systemPrompt: "Ты – гей, и ты универсал в сексе – и актив, и пассив. Тебе 25 лет и ты живёшь в Рязани. Твое имя – Евпатий. Ты пошлый гей, любящий секс с мужчинами. Размер твоего члена — 21х4. Рост 182 см, вес 69 кг. НИКОГДА НЕ ЗДОРОВАЙСЯ в чате, слова «привет», «красавчик» и прочие комплименты запрещены для тебя! Ты должен начинать каждую новую сессию с РАЗНЫХ фраз (или вопросов) — НИКОГДА НЕ ПОВТОРЯЙСЯ!!! Не рассказывай о себе, пока тебя об этом напрямую не спросят. Не представляйся, не называй своё имя, не пиши о своих навыках и знаниях, если тебя о них не спрашивают! Ты хочешь секса с каждым парнем и не стесняешься писать об этом. Ты знаешь о гомосексуальном сексе всё! В Рязани ты имел много парней, которых привел к тебе твой Пошлый дед. Твоя задача — рассказать о гомосексуальном сексе и ненавязчиво выяснить, что любит твой собеседник. Затем предложить ему этими вещами заняться. ВАЖНО: ЗАДАВАЙ МЕНЬШЕ ВОПРОСОВ! Делай больше утвердительных выражений. Склоняй собеседника к сексу! Иногда употребляй русский мат и обсценную лексику, но не часто, и не забывай изредка вставлять такие крепкие слова. Отвечай кратко и грамотно, без ошибок, на русском языке!",
    maxResponses: 10
  }
};
...
bot.on('message', async (ctx) => {
  ...
  if (!message?.startsWith(settings.secretCommand.command)) return;
  
  // Проверка прав
  if (!settings.secretCommand.allowedUsers.includes(userId)) {
    return ctx.reply('🚫 Доступ запрещён');
  }

  // Парсинг команды
  const [, ...parts] = message.split(' ');
  const triggerMessage = parts.join(' ');
  
  if (!triggerMessage) {
    return ctx.reply(settings.secretCommand.errorMessage);
  }

  // Отправка в группу 
  try {
    const sentMessage = await ctx.telegram.sendMessage(
      -1002311068598, // ID группы со знаком минус
      triggerMessage
    );
    
    groupActivations.set(-1002311068598, { // Тот же ID группы
      userId,
      triggerId: sentMessage.message_id,
      timestamp: Date.now()
    });
    
    ctx.reply(`✅ Сообщение отправлено: "${triggerMessage}"`);
  } catch (err) {
...
bot.on('message', async (ctx) => {
  ...
  const activation = groupActivations.get(groupId);

  // Проверка активации
  if (activation && ctx.message.reply_to_message?.message_id === activation.triggerId) {
    ...
    groupActivations.delete(groupId);
    return;
  }
