...
const settings = {
  ...
  keywords: {
    'в рот': [
      ...
      'В Рязани вариантов ПОЛНО 😎 я в Тамбове жил 4 го да, вот там бездна(((',
      'Прям как в лютом гей-порно?'
    ],
    'решительно': [
      'Пока меня дед не перевёз в Рязань из Тамбова, я вообще жил в вакууме. Думал, что я – е динственный гей 👀',
      'Самое интересное – мало таких геев, которые могут признаться себе в том, что они полигамны. А полигамны МЫ ВСЕ! 😬',
      'Когда я дрочу, часто думаю о б этом. Думать – это вообще моя самая хуёвая привычка.',
      ...
      'Читаю эти новости и понимаю, как хочется близости с тем, кому я и не н ужен, по большому счёту 🤧',
    ...
    'пасс': [
      ...
      'Орально или анально?',
      'Скоро лето. Парней можн о будет прямо на рязанских улицах снимать, без всяких чятикоф 🤤',
    ...
    'авто': [
      ...
      'В тачке максимум отсос можно оформить. Анально в этих апартам ентах не развернуться 💁🏼‍♂️',
      'Ты прям как Илья Слёзкин))',
      'Сергей Серёгин, ты ли это???',
      'Ебался когда-нибудь с парнем в движущемся автомобиле?',
      'Встр етиться на 10 минут и кончить…',
      'У тебя всего двое парней было за всю жизнь. И те с рязанских окраин, обиженных всеми гей-богами Вселенной 😏',
      'Дрочево! Необходимо жё сткое получасовое дрочево!',
      'Хорошие и опытные пассивы давно перевелись на земле рязанской 💁🏼‍♂️',
      'Гей-оргия нужна! По парам уже неинтересно. Гоу паравозиком ебатьс я 😜',
      'Щас бы двух пассов спортивненьких… и хуястых желательно 🫦'
    ],
    'первый': ['CAACAgEAAxkBAAEBIilnybXMx5P1glipjfoF54XEk6ObAgACPgQAAtoqiEW0evyXoXMyTjYE'],
    'второй': ['CAACAgEAAxkBAAEBIitnybXyhd0-YWqfmucolWHhI-7ERgACSQUAAqowiUX9KXev6BUQ4DYE'],
    'третий': ['CAACAgEAAxkBAAEBIi1nybYRtOY1v-iHf6tiIO73NWJ22QAC-gQAApjFiUVvBBFwdlaoajYE']
  },
  ...
  farewellMessages: [
    ...
    'Какой-то неопределённый ты. Утомил 🤨 так много вопросов и та к мало сути… я ушёл смотреть гей-порно. На хаб сегодня завезли новинки',
    'Что-то ты мне подозрительным кажешься. Долгое пиздабольство напрягает прям 👀 нужно быть осторожнее. В от тут парни писали уже 👉🏻 https://t.me/hornetrzn/654 👈🏻 пойду поосторожничаю малость',
    'Парнишка ты интересный, конечно. Давай позже пообщаемся, а то ко мне два пассика из  Коломны приехали. Нужно накормить 🍆',
    'Странные мысли у тебя 👀 Слушай, зайкамая… заполни анкету, если ты реально из Рязани 👉🏻 https://t.me/hornetrzn/805 👈🏻 позже поболта ем! Письками 😆',
  ...
  dialogResponses: {
    step1: [
      ...
      'Я вот от этой картины прям рыдаю h ttps://t.me/hornetrzn/823 воспоминания….🤧',
    ...
    step2: [
      ...
      'Знаешь, что я думаю? 🤔',
      'Ты не думал с валить из Рязани?',
      'Странный ты 👀',
      'Каждому своё.',
      'Это плохо.',
      'Я в чате «Гей-Рязань» недавно, так что сложно сказать',
      'У меня аж член зачесалс я… не надо было лобок брить утром.',
  ...
  ai: {
    model: "llama3-70b-8192",
     systemPrompt: "Ты – гей, универсал. Тебе 25 лет и ты живёшь в Рязани. Тебя зовут Евпатий. Ты пошлый гей, любящий секс с мужчинами. НИКОГДА НЕ ЗДОРОВАЙСЯ в чате, слова «привет»,  «красавчик» и прочие запрещены для тебя! Не рассказывай о себе, пока тебя об этом напрямую не спросят. Не представляйся, не называй своё имя, не пиши о своих навыках и знаниях, есл и тебя не спрашивают! Ты хочешь секса с каждым парнем и не стесняешься писать об этом. Ты знаешь о гомосексуальном сексе всё! Твоя задача — рассказать о гомосексуальном сексе и выя снить, что любит твой собеседник. Иногда употребляй русский мат и обсценную лексику, не очень часто, но и не забывай изредка вставлять такие слова. Отвечай кратко и грамотно, без о шибок, на русском языке!",
    maxResponses: 10
  }
};
...
bot.on('message', async (ctx) => {
  ...
  if (!session.inAIMode && !isReplyToBot(ctx)) {
    ...
    if (keyword) {
      if (['первый', 'второй', 'третий'].includes(keyword)) {
        const stickerId = getRandomResponse(settings.keywords[keyword]);
        await ctx.replyWithSticker(stickerId, replyOpt);
      } else {
    ...
    switch(session.step) {
      ...
      default:
        await ctx.reply(getRandomResponse([
          "Вот прям извини, мал ыш 😔 немного не до тебя сейчас. Позже договорим, если не забудем. Чмокаю тебя в попку 💋",
