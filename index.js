...
bot.command('etonensecret', async (ctx) => {
  ...
  try {
    if (file) {
      // Отправляем медиа в целевой чат
      await ctx.telegram.sendMediaGroup(TARGET_CHAT_ID, [{
...
// Вспомогательная функция для определения типа медиа
function getMediaType(file) {
  if (file.hasOwnProperty('file_name')) return 'document';
  if (file.hasOwnProperty('duration')) return 'video';
  if (file.hasOwnProperty('performer')) return 'audio';
  return 'photo';
}
